...
import { usePlaylists, Playlist } from '@/contexts/PlaylistContext'; // Importar el tipo Playlist
...
export default function PlaylistDetailScreen() {
  ...
  const { getPlaylistById, removeSongFromPlaylist, loadPlaylists, loading: playlistsLoading } = usePlaylists();
  ...
  const loadData = async () => {
    try {
      setRefreshing(true);
      await loadPlaylists(); // Recargar todas las playlists para asegurar datos frescos
    } finally {
      setRefreshing(false);
    }
  };

  // Obtener la playlist directamente del contexto
      ...
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            if (success) {
              // La UI se actualiza automáticamente porque el estado está en el contexto
              console.log('✅ Canción eliminada, la UI se actualizará.');
            }
          },
        },
      ]
  ...
  if (!playlist) {
    return (
      <View style={styles.loadingContainer}>
        <Text style={styles.errorText}>Playlist no encontrada o no te pertenece.</Text>
        <TouchableOpacity onPress={() => router.back()}><Text style={{ color: '#8b5cf6', marginTop: 10 }}>Volver</Text></TouchableOpacity>
      </View>
    );
  }
